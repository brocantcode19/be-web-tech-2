...
export const updateStudent = async (c: Context) => {
  try {
    ...
    const [existingStudent] = await pool.query<Student[] & RowDataPacket[]>(
      "SELECT * FROM students WHERE id = ?",
      [id]
    );
    ...
    const current = existingStudent[0];
    ...
    const course = data.course ?? current.course;
    const year_level = data.year_level ?? current.year_level;
    const gpa = data.gpa ?? current.gpa;
    const enrollment_status = data.enrollment_status ?? current.enrollment_status;
